...
import { loginUser } from '../../utils/django/django.utils.js'
...
const SignIn = () => {
  ...
  const onSubmitHandler = async (e) => {
    e.preventDefault();
    const currentUser = await loginUser(formData.username, formData.password);
  ...
  return (
    <div className='flex flex-col justify-center w-screen min-h-screen gap-8 bg-center bg-no-repeat bg-cover' style={{ backgroundImage: `url(${SignInBg})` }}>
      ...
      <h1 className='text-4xl font-bold text-center lg:text-6xl text-secondary'>Welcome back!!!!</h1>
        ...
        <div className='p-8 shadow-xl bg-black/90 shadow-black/50'>
          ...
          <form className='flex flex-col gap-5 mt-8' onSubmit={onSubmitHandler}>
            <div className='flex flex-col gap-4 md:flex-row md:justify-between'>
              ...
              <input className='text-xl bg-transparent border-0 border-b-2 focus:bg-transparent text-white/70 focus:outline-none focus:text-white' placeholder='Enter your username' name='username' onChange={handleChange} />
            </div>
            ...
            <button type='submit' className='text-secondary hover:text-white w-fit relative -right-[50%] -translate-x-1/2'>Login</button>
            <p className='text-lg text-white'>Don't have a account? <a href='/sign-up' className='text-secondary hover:text-white'>Sign Up</a></p>
          </form>
        </div>
      </div>
    </div>
